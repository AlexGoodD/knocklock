...
class _TokenModalState extends State<TokenModal> {
  ...
  Future<void> _generateAndSetPassword() async {
    ...
    final random = Random();
    final generatedPassword = List.generate(8, (index) => characters[random.nextInt(characters.length)]).join();

    // Precargar la contraseña generada en el InputPassword
    _passwordKey.currentState?.clear();
    for (int i = 0; i < generatedPassword.length; i++) {
      _passwordKey.currentState?.onChanged(generatedPassword.substring(0, i + 1));
    }

    print('La contraseña generada es: $generatedPassword');
    // Hashear la contraseña generada
    final hashedPassword = sha256.convert(utf8.encode(generatedPassword)).toString();
